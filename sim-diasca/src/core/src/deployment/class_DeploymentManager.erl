...
construct( State,
	...
	io:format("~w~n", [DeploymentSettings]),

	io:format("~w~n", [StartingState]),
	...
	case ?getAttr(shutdown_initiated) of
		...
		false ->
			?fatal_fmt( "error exit signal received ('~p'), "
						"performing now an emergency simulation shutdown. Was sent by ~p",
						[ ExitType, PidOrPort ] ),
